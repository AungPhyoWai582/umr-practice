...
import SpecialitiesSetupModal from "../../modals/SpecialitiesSetupModal";

const SpecialitiesList = ({ toggleSidebar }) => {
  const [specialitiesData, setSpecialitiesData] = useState([]);
  const [toggleIndex, setToggleIndex] = useState();
  const [show, setShow] = useState(false);
  const [specilityInput, setSpecilityInput] = useState();
    ...
    axiosConfig
      ...
      .then((response) => {
        if (response.data.data) {
          setSpecialitiesData(response.data.data);
          setRefresh(false);
          setShow(false);
        }
      })
  ...
  const ActiveControl = (e, id) => {
    ...
    let index = id - 1;
    let obj = {
      id: specialitiesData[index].id,
      name: specialitiesData[index].name,
      is_active:
        specialitiesData[index].is_active === "true" ? "false" : "true",
    };
    console.log(obj);
    axiosConfig
      .put(`specialities/${id}`, obj)
      .then((response) => {
        if (response.data.data) {
          setRefresh(true);
          setShow(false);
  ...
  const Edit = (e, id) => {
    e.preventDefault();
    setTempId(id);
    setSpecilityInput(specialitiesData[id - 1].name);
    setTemIsActive(specialitiesData[id - 1].is_active);
    setOpenModal(true);
    setEdit(true);
  };
  ...
  const Update = (e) => {
    ...
    axiosConfig
      .put(`specialities/${tempId}`, obj)
      .then((response) => {
        if (response.data.data) {
          setRefresh(true);
          setOpenModal(false);
          setSpecilityInput(null);
          setTemIsActive(null);
          setEdit(false);
          setShow(false);
  ...
  let showOutput = search
    ? specialitiesData.filter((td) => td.name.toLowerCase().includes(search))
    : specialitiesData;
  return (
    <React.Fragment>
      <Topbar toggleSidebar={toggleSidebar} pageTitle="Specialities List" />
      ...
      {openModal && (
        <SpecialitiesSetupModal
          setOpenModal={setOpenModal}
          specilityInput={specilityInput}
          setSpecilityInput={setSpecilityInput}
          Create={Create}
          Update={Update}
          edit={edit}
          setEdit={setEdit}
